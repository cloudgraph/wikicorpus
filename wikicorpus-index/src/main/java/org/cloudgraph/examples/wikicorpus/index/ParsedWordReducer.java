...
public class ParsedWordReducer extends
    GraphReducer<Text, LongWritable, Text> {
	private static Log log = LogFactory.getLog(ParsedWordReducer.class);
	...
	private void process(Text key, long sum, Context context) throws IOException {
		...
		if (graphs == null || graphs.length == 0) {
			...
			Type rootType = PlasmaTypeHelper.INSTANCE.getType(ParsedWordAggregate.class);
			ParsedWordAggregate wordAggregate = (ParsedWordAggregate) dataGraph.createRootObject(rootType);
			...
			dataGraph = graphs[0];
			dataGraph.getChangeSummary().beginLogging();
			ParsedWordAggregate wordAggregate = (ParsedWordAggregate) dataGraph.getRootObject();
			wordAggregate.setWordCount(sum);
		}
